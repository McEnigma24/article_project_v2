...
class Multi_Dimension_View_Array
{
    ...
    GPU_LINE(__host__) bool bound_check(u64 x) const { return (0 < x && x < buffer_size); }
...
public:
    ...
    GPU_LINE(__host__) u64 get_total_number() const { return buffer_size; }
    ...
    T* get(u64 x)
    {
        BOUND_CHECKS_Multi_Dimension_View_Array(if (!(x < buffer_size)) FATAL_ERROR("accesing beyond vector bounds");)
    ...
    {
        BOUND_CHECKS_Multi_Dimension_View_Array(if (!(index_1d < buffer_size)) FATAL_ERROR("accesing beyond vector bounds");)
